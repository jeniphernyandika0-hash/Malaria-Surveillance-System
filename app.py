...
def plot_scatter_geo(df, lat="lat", lon="lon", color=None, size=None, hover_name=None,
                     hover_data=None, color_discrete_map=None, zoom=8, height=480, title=None):
    """
    Map that works on Streamlit Cloud without a Mapbox token.
    Tries plotly scatter_map (new), then scatter_mapbox + open-street-map, then plain scatter.
    """
    d = df.dropna(subset=[lat, lon]).copy() if df is not None and not df.empty else pd.DataFrame()
    if d.empty:
        return None
    kwargs = dict(
        lat=lat, lon=lon, zoom=zoom, height=height,
    )
    ...
    try:
        ...
        pass
    # 2) Mapbox style open-street-map (often works without token)
    try:
        fig = px.scatter_mapbox(d, **kwargs)
        fig.update_layout(mapbox_style="open-street-map", margin=dict(l=0, r=0, t=40 if title else 10, b=0))
        return fig
    except Exception:
        pass
    # 3) Fallback: simple lat/lon scatter (always works)
    sk = {}
    if color is not None:
        sk["color"] = color
    if size is not None:
        sk["size"] = size
    if hover_name is not None:
        sk["hover_name"] = hover_name
    if color_discrete_map is not None:
        sk["color_discrete_map"] = color_discrete_map
    fig = px.scatter(d, x=lon, y=lat, **sk, height=height, title=title or "Facility locations (map unavailable)")
...
- Not a substitute for field investigation or EPR guidelines.
    ...
    with tabs[1]:
        ...
        if f4.empty:
            ...
            # size must be positive for plotly
            f4 = f4.copy()
            f4["risk_score"] = pd.to_numeric(f4.get("risk_score"), errors="coerce").fillna(0.1).clip(lower=0.05)
            fig = plot_scatter_geo(
                f4,
                lat="lat", lon="lon",
                color="risk_level",
                size="risk_score",
                ...
                zoom=8,
                height=480,
                title="4-week EW risk by facility",
            )
            if fig is not None:
                st.plotly_chart(fig, use_container_width=True)
                st.caption("Bubble size ∝ risk score. Colour = risk level (4-week horizon).")
            else:
                st.info("No facility coordinates for map.")
